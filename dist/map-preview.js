...
(async()=>{
 ...
 try{
 const mapName=new URLSearchParams(location.search).get('map')||'ancientdungeon_red_1_1';
 ...
 const visible=new Set(data.layers.map((_,i)=>i));let selected=null,width=0,height=0,scale=20,fitScale=20,ox=0,oy=0,drag=null;
 ...
 $('#layers').innerHTML=data.layers.map((l,i)=>`<label><input type="checkbox" data-layer="${i}" checked ${l.tiles.length?'':'disabled'}><span class="swatch" style="background:${colors[l.name]||'#aaa'}"></span>${esc(l.name)}<small>${l.tiles.length}</small></label>`).join('');
 $('#objects').innerHTML+=[...tiles].sort((a,b)=>a.layer-b.layer).map(t=>`<option value="${t.id}">${esc(data.layers[t.layer].name)} / ${esc(t.name)} (${t.position[0]}, ${t.position[2]})</option>`).join('');
 ...
 function draw(){
  ...
  if(selected&&visible.has(selected.layer)){const [x,y]=point(selected);ctx.strokeStyle='#ffe5a0';ctx.lineWidth=2;ctx.strokeRect(x-scale*.55,y-scale*.55,scale*1.1,scale*1.1)}
 ...
 function select(t){selected=t;$('#objects').value=t?t.id:'';if(t){const l=data.layers[t.layer];$('#inspector').innerHTML=`<h3>${esc(t.name)}</h3><dl><dt>Layer</dt><dd>${esc(l.name)}</dd><dt>Tileset</dt><dd>${esc(t.tileset)}</dd><dt>X / Y / Z</dt><dd>${t.position.join(' / ')}</dd><dt>Rotation code</dt><dd>${t.rotation}</dd><dt>Partition</dt><dd>${t.partition.join(', ')}</dd><dt>Byte offset</dt><dd>${t.offset} (0x${t.offset.toString(16)})</dd></dl>`}else{$('#inspector').innerHTML='<p class="empty">Select a tile to see its name, position and source record.</p>'}draw()}
 $('#layers').addEventListener('change',e=>{const n=Number(e.target.dataset.layer);e.target.checked?visible.add(n):visible.delete(n);draw()});
 $('#objects').addEventListener('change',e=>{const t=tiles.find(t=>t.id===e.target.value);if(t){visible.add(t.layer);$(`[data-layer="${t.layer}"]`).checked=true;ox=width/2-t.position[0]*scale;oy=height/2+(t.position[2]*(tilted()?.8:1)+t.position[1]*(tilted()?.6:0))*scale}select(t)});
 $('#render-mode').onchange=fit;$('#grid').onchange=draw;$('#fit').onclick=fit;$('#zoom-in').onclick=()=>zoom(1.3);$('#zoom-out').onclick=()=>zoom(1/1.3);
 ...
 canvas.addEventListener('pointerup',e=>{if(drag&&!drag.moved){const [x,y]=pos(e);const hits=ordered.filter(t=>{const [a,b]=point(t);return visible.has(t.layer)&&Math.abs(x-a)<scale*.5&&Math.abs(y-b)<scale*.5});select(hits.at(-1)||null)}drag=null});
