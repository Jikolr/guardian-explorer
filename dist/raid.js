...
function resultsUI(){
 ...
 $('#damage-results').innerHTML=r.heroes.map(v=>{
  ...
  const rows=v.results.map(a=>{
   const title=`${esc(a.name)}<small>${esc(a.kind==='weapon'?'Weapon skill':a.kind==='chain'?'Chain skill':a.kind==='leader'?'Leader skill':a.kind)} <span class="badge ${a.status}">${a.eventOnly?'Code-traced event':a.status==='verified'?'Code-traced formula':a.status}</span></small>`;
