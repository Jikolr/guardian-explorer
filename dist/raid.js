...
function activeBuffUI(){
 ...
 const labels={battleAtk:'battle ATK',skill:'skill damage',crit:'critical chance',critDamage:'critical damage',ranged:'ranged damage',melee:'melee damage',normal:'normal attack damage',element:'elemental ATK'};
...
function resultsUI(){
 ...
 $('#damage-results').innerHTML=r.heroes.map(v=>{
  ...
  const rows=v.results.map(a=>{
   const title=`${esc(a.name)}<small>${esc(a.kind==='weapon'?'Weapon skill':a.kind==='chain'?'Chain skill':a.kind==='leader'?'Leader skill':a.kind)} <span class="badge ${a.status}">${a.status==='verified'?'Recovered formula':a.status}</span></small>`;
   ...
   return `<tr class="${a.hits.length>1?'total-row':''}"><td>${title}${a.hits.length>1?'<small>Full cast total</small>':''}</td><td>${fmt(a.total.noncrit)}</td><td>${fmt(a.total.critical)}</td><td>${fmt(a.total.average)}</td></tr>`+(a.hits.length>1?a.hits.map((hit,i)=>`<tr><td>↳ Hit ${i+1}${a.timings?' · '+a.timings[i].toFixed(2)+' s':''}</td><td>${fmt(hit.noncrit)}</td><td>${fmt(hit.critical)}</td><td>${fmt(hit.average)}</td></tr>`).join(''):'');
  }).join('');
  const breakdown=v.results.filter(a=>a.total).map(a=>`<details><summary>${esc(a.name)} · calculation</summary><p class="fine">${esc(a.note)}</p>${a.breakdown.map(c=>`<div class="formula">${esc(c.element)} weapon ATK after matching bonus: ${prec(c.weaponAttack)}
...
$('#download-results').onclick=()=>download('guardian-raid-results.json',{snapshot:E.db.snapshot,setup:state,results:lastResult.heroes.map(v=>({hero:v.h.Id,preparedAtk:v.atk,crit:v.crit,critMultiplier:v.critMultiplier,attacks:v.results,warnings:v.warnings}))});
