...
for addr,r in mapped.items():
 if not addr or r['type']!='CharacterTool.WhileCollisionCalculate' or r['name'] not in ['TriggerAction','get_Each','get_True']:continue
 end=min(a for a in mapped if a>addr)
 out=n.disasm(addr,end-addr)
 (root/'docs/passive-evidence'/('WhileCollisionCalculate-'+r['name']+'.asm')).write_text(out,encoding='utf-8')
 print(r['type'],r['name'],hex(addr),out)
