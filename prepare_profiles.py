...
for origin,variants in groups.items():
 ...
 info=dict(id=origin,name=display,internal=key,evidence=evidence,status='Confirmed' if key in manual else 'English spelling match' if display else 'Unmapped',variants=variants,image=media.get('heroes:'+str(base['id']),{}).get('image'),profile=profiles.get(origin),biography=bio['fields']['Text'] if bio else None,biographyId=bio['id'] if bio else None,biographyNote='Matched by name at the start of the English biography; story-key mapping is not decoded.',skills=skills,battleStyles=battle_styles,weapons=linked)
 result.append(info)
 for r in variants:
